...
class StableDiffusion(nn.Module):
    ...
    def train_step(self, 
        ...
        if image_prompt is not None:
            prompt_latents = self.encode_imgs(image_prompt).to(self.device)
        ...
        with torch.no_grad():
            ...
            if image_prompt is not None:
                noise_pred += img_prompts_coef * (latents - prompt_latents)
    ...
    def train_step_multi(self,
        ...
        for i in range(len(multi_embeddings)):
            embedding = multi_embeddings[i]
            control_image = controls[i]
            control_image = control_image.to(self.device)
            
            control_image_512 = F.interpolate(control_image,(512, 512), mode='bilinear', align_corners=False)
            control_image_latents = self.encode_imgs(control_image_512).to(self.device) #把control_image编码到特征空间
            with torch.no_grad():
                # add noise
                noise = torch.randn_like(latents).to(self.device)
                #This is the only place we use the scheduler...the add_noise function. What's more...it's totally generic! The scheduler doesn't impact the implementation of train_step...
                ...
                # pred noise
                latent_model_input = torch.cat([latents_noisy] * 2)
                noise_pred = self.predict_noise(latent_model_input, embedding, t)
                if control_image is not None:
                    noise_pred += (latents - control_image_latents) * control_image_coef
                latent_pred = self.remove_noise(latents_noisy, noise_pred, t)
                output = latent_pred
            ...
            #增加对control_image的响应
            '''
            control_image_delta = latent_pred - control_image_latents
            total_delta = total_delta + control_image_delta * control_image_coef
            '''
            output=torch.stack([*output, *latent_pred])
            
            if image_coef: # 有图片condition作为指导
                # Ryan's Image Guidance
                image_pred_uncond, image_pred_text = image_pred.chunk(2)
                image_pred = image_pred_uncond + guidance_scale * (image_pred_text - image_pred_uncond)
                image_delta=image_pred - pred_rgb_512
